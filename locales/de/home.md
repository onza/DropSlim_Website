---
heroTag: macOS & Windows · lokal · privat · kostenlos · open source
heroTitle: |-
  Große Dateien
  *klein* machen.
heroSub: |-
  Bilder rüberziehen. DropSlim komprimiert — optional Format und Maße.
  Alles bleibt auf deinem Rechner — kein Account, kein Server.
lessVolume: weniger Volumen (+/-)
whyHeading: Darum DropSlim.
compareBeforeAria: 'Vorher: 12,4 MB — 4 Screenshots, eine Mail, Limit erreicht'
compareAfterAria: 'Nachher: 2,1 MB — gleicher Inhalt, passt locker rein'
before: vorher
after: nachher
compareBeforeDesc: 4 Screenshots, eine Mail — Limit erreicht
compareAfterDesc: gleicher Inhalt, passt locker rein
features:
  - num: '01'
    title: Komprimierung
    body: Große Bilddateien werden lokal kleiner — für Mail, Web und Archiv, ohne Qualitätsdrama.
  - num: '02'
    title: Format konvertieren
    body: JPEG, PNG, WebP und AVIF konvertieren — oder Original behalten. HEIC auf macOS. GIF und SVG bleiben unverändert.
  - num: '03'
    title: Max. Breite & Höhe
    body: Pixel-Limits in den Settings setzen. Zu große JPEG, PNG, WebP und AVIF werden proportional verkleinert.
  - num: '04'
    title: Drag & Drop
    body: Dateien oder Ordner aufs Fenster. Mehr brauchst du nicht.
  - num: '05'
    title: Klein und schnell
    body: Leichtgewichtige App, schnelle Komprimierung. Wenig Speicher, kein Ballast.
  - num: '06'
    title: Läuft offline
    body: Kein Internet nötig. Komprimierung läuft komplett auf deinem Rechner.
  - num: '07'
    title: Datenschutz
    body: Kein Upload. Kein Tracking. Deine Fotos verlassen den Rechner nicht.
  - num: '08'
    title: "Stapel\u00ADverarbeitung"
    body: 200 Screenshots? Ein Zug. Ordner rüberziehen, alles in einem Durchlauf.
  - num: '09'
    title: PNG · JPEG · GIF · SVG · WebP · HEIC · AVIF
    body: Alle gängigen Bildformate — Presets für Web, Mail, Archiv. HEIC nur auf macOS.
  - num: '10'
    title: Einstellungen
    body: Überschreiben, daneben mit `.min`-Suffix oder in eigenem Ordner — du legst fest, was passiert.
  - num: '11'
    title: Mehrsprachig
    body: Deutsch, English, Français, Español, Italiano, 日本語 und Português — oder Systemsprache.
  - num: '12'
    title: Open Source
    body: MIT-Lizenz. Quellcode auf <a href="https://github.com/onza/DropSlim" target="_blank" rel="noopener noreferrer">GitHub<span class="visually-hidden"> (öffnet in neuem Tab)</span></a> — einsehbar, forkbar, mitmachbar.
    rich: true
---

## Für alle, die Bilder verschicken & uploaden.

Mail-Anhang voll? Notion langsam? Slack meckert? Website-Pflege? Unkomprimiert sind Grafikdateien weder nachhaltig noch performant. DropSlim macht aus PNG, JPEG, GIF, SVG, WebP, HEIC und AVIF versandbare und webfertige Dateien — optional mit Formatwechsel oder max. Maßen, auf Mac und Windows, ohne dass du in Photoshop irgendwas exportieren musst.
